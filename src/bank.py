...
class Bank(object):
    ...
    def __str__(self):
        ...
        text += "    <parameter type='changing' name='rhoFinancial' value='" + str(
            self.rhoFinancial) + "'></parameter>\n"
    ...
    def get_parameters_from_file(self, bankFilename, state, numBanks, time):
        try:
            ...
            for sub_element in element:
                if sub_element.attrib['type'] == 'changing':
                    name = sub_element.attrib['name']
                    value = sub_element.attrib['value']
                    validFrom = sub_element.attrib['validity'].rsplit("-")[0]
                    validTo = sub_element.attrib['validity'].rsplit("-")[1]
            ...
            logging.error(f'ERROR: {bankFilename} could not be parsed')
    ...
    def update_maturity(self):
        for transaction in self.accounts:
            if int(transaction.transactionMaturity) > 0:  # reduce maturity if duration longer than 0
                ...
                if ((transaction.transactionType == "I") and (
                        int(transaction.transactionTimeOfDefault) > 0)):  # only investments have a time of default
    ...
    def update_risk_aversion(self, state, time):
        ...
        if lastInsolvency[1] == (time - 1) or lastInsolvency[1] == time:
            ...
            self.theta = round(self.theta * (1.0 + state.riskAversionAmplificationFactor),
                               3)  # we round to be on the safe side with float numbers
        else:  # if not, decrease risk aversion
            self.theta = round(self.theta * (1.0 - state.riskAversionDiscountFactor),
                               3)  # we round to be on the safe side with float numbers
            ...
            if transaction.transactionType == type:
                if transaction.transactionType == "I":  # check if we have an investment
                    if transaction.transactionTimeOfDefault == 0:  # check if the investment defaulted
                        ...
                        #
                        self.reduce_banking_capital(
                            transaction.transactionValue)  # reduce the banking capital if the loan defaults
                #
                #
                #
    ...
    # -------------------------------------------------------------------------
    def liquidate_due_transactions(self, type_):
        volume = 0.0
        residual = []

        for transaction in self.accounts:
            if (transaction.transactionType == type_) and (int(transaction.transactionMaturity) == 0):
    ...
    # -------------------------------------------------------------------------
    # def get_ new_deposits(scaleFactor)
    ...
    def get_new_deposits(self, scaleFactor):
        ...
        for transaction in self.accounts:
            if transaction.transactionType == "D":
                ...
                newValue = max(round((1.0 - scaleFactor + 2.0 * scaleFactor * random.random()) * oldValue, 4),
                               0.0)  # make sure we have only positive deposit levels
                transaction.transactionValue = newValue

        returnValue = round(newValue - oldValue,
                            4)  # convention is that outflows are negative, this is the bank's viewpoint
        ...
        if (self.active > -1) and (I > 0.0):
            if round(float(BC / I), 4) < required_capital_ratio:
                ...
                if debug == "info" or debug == "debug":
                    ...
                    logging.info("    time: %s: <bank %s is insolvent: %s < %s>", time, self.identifier,
                                 capital_adequacy, required_capital_ratio)
    ...
    def liquidate_assets(self, initial_assets, current_assets, state, debug, time):
        ...
        if self.Lp < 0.0:  # we have a shortfall in liquidity
            ...
            if self.Lp < 0.0:
                ...
                liquidation_price = math.exp(-liquidation_discount_factor * (
                        (initial_assets - current_assets + liquidation_volume) / (initial_assets)))

                # now loop over all investments and liquidate as many as necessary
                for transaction in self.accounts:
                    if (transaction.transactionType == "I") and (liquidated_assets < liquidation_volume) and (
                        ...
                        self.reduce_banking_capital((
                                                            1.0 - liquidation_price) * transaction.transactionValue)  # but suffer a loss from costly liquidation
                        transaction.transactionValue = 0.0
                        # check if we are still alive
                        self.check_solvency(state, debug,
                                            time)  # note that this has to be done before purge_accounts (afterwards, I=0)
    ...
    # -------------------------------------------------------------------------
    def calculate_optimal_investment_volume(self, state):
        # TODO this is not a good name, better would be calculate_optimal_portfolio
    ...
    def get_initial_banking_capital(self, required_capital_ratio):
        ...
        # return round((value + new_value), 4)
        return value
    # -------------------------------------------------------------------------
    ...
    def get_account(self, type_):
        ...
        for transaction in self.accounts:
            if transaction.transactionType == type_:
                volume = volume + float(transaction.transactionValue)
