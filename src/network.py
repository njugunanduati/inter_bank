...
class Network(object):
    ...
    def initialize_networks(self, environment):
        ...
        if (environment.parameters.graphType == "list"):
            ...
            try:
                ...
                contracts = nx.read_weighted_edgelist(
                    str(environment.parameters.contractsNetworkFile) + ".list").to_directed()
                logging.info("  read network of contracts: %s", environment.parameters.contractsNetworkFile)
            except:
                logging.error("    ERROR: no .list contractsNetworkFile found: %s",
                              environment.parameters.contractsNetworkFile)
        ...
        if (environment.parameters.graphType == "gexf"):
            ...
            try:
                ...
                logging.error("    ERROR: no .gexf contractsNetworkFile found: %s",
                              environment.parameters.contractsNetworkFile)
                logging.error("    ERROR: check your %s.xml file.", environment.parameters.identifier)
        ...
        logging.info("  created the network of contracts with %s nodes and %s links", str(len(self.contracts.nodes())),
                     str(len(self.contracts.edges())))
    ...
    def do_interbank_trades(self, state):
        ...
        random.shuffle(list(banks))
        for bank in banks:  # loop over all banks in the list of shuffled banks
            # print(f'bank --- {str(bank.identifier)}  {str(bank.Lp)}')
            ...
            for neighbor in neighbors:
                # print(f'neighbor --- {bank.Lp * neighbor.Lp}')
                # now check if we have a match
                if (bank.Lp * neighbor.Lp < 0.0) or (bank.active > -1) or (neighbor.active > -1):
                    ...
                    if bank.Lp > 0.0:  # bank has excess liquidity
                        # add transactions
                        bank.add_transaction("L", int(bank.identifier), int(neighbor.identifier), value, interest,
                                             maturity, timeOfDefault)
                        neighbor.add_transaction("L", int(bank.identifier), int(neighbor.identifier), value, interest,
                                                 maturity, timeOfDefault)
                    ...
                    if neighbor.Lp > 0.0:  # neighbor has excess liquidity
                        # add transactions
                        bank.add_transaction("L", int(neighbor.identifier), int(bank.identifier), value, interest,
                                             maturity, timeOfDefault)
                        neighbor.add_transaction("L", int(neighbor.identifier), int(bank.identifier), value, interest,
                                                 maturity, timeOfDefault)
                        # update network of exposures
    ...
    def remove_defaulted_loans(self, bank, time):
        ...
        if num_neighbors > 0:
            logging.info("      time: %s: contagion originating from bank %s to %s neighbors", time, bank.identifier,
                         num_neighbors)
    ...
    def __str__(self):
        ...
        for fromID, toID, edata in self.exposures.edges(data=True):
            text += "  <edge from='" + fromID.identifier + "' to='" + toID.identifier + "' weight='" + str(
                edata['weight']) + "'>\n"
        text += "</network>\n"
